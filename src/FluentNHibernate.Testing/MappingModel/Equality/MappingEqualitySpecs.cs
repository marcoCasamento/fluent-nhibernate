...
namespace FluentNHibernate.Testing.MappingModel.Equality
{
    ...
    public class when_comparing_two_identical_ArrayMappings : MappingEqualitySpec<CollectionMapping>
    {
        public override CollectionMapping create_mapping()
        {
            var mapping = new CollectionMapping(new DummyPropertyInfo("name", typeof(Target)).ToMember())
            {
                ...
                OtherSide = new CollectionMapping(null), Persister = new TypeReference(typeof(Target)), Relationship = new ManyToManyMapping(null),
    ...
    public class when_comparing_two_identical_ManyToOneMappings : MappingEqualitySpec<ManyToOneMapping>
    {
        ...
        {
            var mapping = new ManyToOneMapping(new DummyPropertyInfo("prop", typeof(Target)).ToMember())
            {
                Access = "access",
                Cascade = "cascade",
                Class = new TypeReference(typeof(Target)),
    ...
    public class when_comparing_two_identical_PropertyMappings : MappingEqualitySpec<PropertyMapping>
    {
        ...
        {
            var mapping = new PropertyMapping(new DummyPropertyInfo("prop", typeof(Target)).ToMember())
            {
                Access = "access",
                ContainingEntityType = typeof(Target),
                Formula = "formula",
    ...
    public abstract class MappingEqualitySpec<T> : Specification
    {
        ...
        protected ComponentMapping create_component()
        {
            return new ComponentMapping(typeof(ExampleClass));
        }

        protected ComponentMapping create_dynamic_component()
        {
            return new ComponentMapping(typeof(ExampleClass));
        }

        protected PropertyMapping create_property()
        {
            return new PropertyMapping(null);
        }
